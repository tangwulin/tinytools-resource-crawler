...
const updateGenshin = async (page, config) => {
    ...
    const result = (await page.$$eval('#CardSelectTr > div', (el) => {
        return el.map((item) => {
            return {
    ...
    const genderResult = (await page.$$eval('#CardSelectTr > tbody > tr', (el) => {
        return el.map((item) => {
            return {
...
const updateArknights = async (page, config) => {
    await page.setViewport({ width: 1080, height: 1024 });
    console.log(config.webUrl);
    await page.goto(config.webUrl);
    ...
    const female = [];
    for (let i = 0; i < 2; i++) {
        const result = (await page.$$eval('#filter-result > div', (el) => {
            return el.map((item) => {
                return {
...
const updateBlueArchive = async (config) => {
    await axios
        ...
        const female = res.data.data.students
            .map((item) => {
            const description = `${item.family_name}${item.given_name}${item.skin.length > 0 ? `(${item.skin})` : ''}`;
...
const updateStarRail = async (page, config) => {
    await page.setViewport({ width: 1080, height: 1024 });
    console.log(config.webUrl);
    await page.goto(config.webUrl);
...
const updateUmamusume = async (page, config) => {
    await page.setViewport({ width: 1080, height: 1024 });
    console.log(config.webUrl);
    await page.goto(config.webUrl);
    await page.waitForSelector('#mw-content-text > div > div.main-line-wrap > div > div > div:nth-child(1) > span:nth-child(132)');
    const female = await page.$$eval('#mw-content-text > div > div.main-line-wrap > div > div > div:nth-child(1) > span', (el) => {
        return el.map((item) => {
            return {
