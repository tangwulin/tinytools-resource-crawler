...
const result = [];
const charaDatas = [];
const start = async (oldCharactersName, threads) => {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        if (i === 0) {
            await page.click('#pagination > div.paginations-container > div.checkbox-container > div:nth-child(2)');
        }
    }
    ...
    await Promise.all(chunks.map(async (items, workerIndex) => {
        ...
        for (const item of items) {
            ...
            workerSpinner.succeed(`${workerIndex + 1}# Worker获取${item}的立绘成功 ${index}/${items.length}`);
            workerSpinner.color = 'green';
            //获取干员语音
            workerSpinner.color = 'yellow';
            workerSpinner.start(`${workerIndex + 1}# Worker正在获取${item}的语音…… ${index}/${items.length}`);
            ...
            const charaKey = await workerPage.$eval('#voice-data-root', (el) => el.getAttribute('data-voice-key'));
            const charaData = {
                name: item,
                charaKey,
                images: charaImgs,
            ...
            workerSpinner.succeed(`${workerIndex + 1}# Worker获取${item}的语音成功 ${index}/${items.length}`);
            workerSpinner.color = 'green';
...
const oldCharactersName = [];
//TODO: 读取旧数据来增量更新
const stopwatch = performance.now();
start(oldCharactersName, 4)
    .then(async () => {
    if (!fs.existsSync('./data/arknights'))
        fs.mkdirSync('./data/arknights', { recursive: true });
    await Promise.all(charaDatas.map(async (charaData) => {
        await fs.promises.writeFile(`./data/arknights/${charaData.charaKey}.json`, JSON.stringify(charaData, null, 2));
    }));
})
    .then(async () => {
    await fs.promises.writeFile('./data/arknightsCharacterList.json', JSON.stringify(result, null, 2));
})
    .then(() => {
    console.log('done');
